...
typedef struct FHBezierSubpath{
    CGPoint startPoint;
...
@implementation UIBezierPath (FHLength)
...
- (void)extractSubpaths:(FHBezierSubpath *)subpathArray{
...
- (CGPoint)pointAtPercent:(CGFloat)t ofSubpath:(FHBezierSubpath)subpath{
...
- (CGFloat)fh_length {
    ...
    FHBezierSubpath subpaths[subpathCount];
...
- (CGPoint)fh_pointAtPercentOfLength:(CGFloat)percent {
    ...
    FHBezierSubpath subpathContainingPoint;
    for (NSUInteger i = 0; i < subpathCount; i++) {
